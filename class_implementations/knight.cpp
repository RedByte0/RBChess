...
std::vector<int> knight::possible_movements() const {
...
void knight::horizontal_movements(std::vector<int>& movements) const {
	for(int horizontal : {2,-2}) {
		int horizontal_position = position() + horizontal;
		if(board_ptr->get_row_from_position(horizontal_position) == board_ptr->get_row_from_position(position())) {
			for(int vertical : {-board_ptr->columns(), board_ptr->columns()}) {
				int vertical_position = horizontal_position + vertical;
...
void knight::vertical_movements(std::vector<int>& movements) const {
	for(int vertical : {board_ptr->columns()*2, -board_ptr->columns()*2}) {
		int vertical_position = position() + vertical;
		if(board_ptr->position_out_of_bounds(vertical_position) == false) {
			for(int horizontal : {1, -1}) {
				int horizontal_position = vertical_position + horizontal;
