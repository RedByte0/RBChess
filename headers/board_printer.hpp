...
private:
	...
	std::vector<int> _highlighted_positions;

	void print_row(bool team, int row, int& board_position) const;
	void print_square_row(bool team, int board_position) const;
	void print_square_row_with_row_number(bool team, int row, int board_position) const;
	...
	bool is_highlighted(int position) const;
...
public:
	...
	void operator=(const board&& b) = delete;
	void highlight_positions(std::vector<int>&& positions);
};
